...
/**
 * Servlet Filter implementation class CarFilter
 */
...
public class CarFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(uri!=null&&(uri.equals(webroot+"/index/error.jsp")))
		{
			...
			if(session.getAttribute("depatment")==null||session.getAttribute("depatment").equals("经理")||session.getAttribute("depatment").equals("销售"))
			{
				chain.doFilter(req, res);				
			}
			else
